...
def test_every_image_the_readme_shows_exists():
    """A README that renders broken images looks abandoned.

    Cheap to get wrong: the galleries are generated by a script, so a renamed
    sheet or a forgotten run leaves a dead <img> that nobody notices until
    someone lands on the project page.
    """
    text = (ROOT / "README.md").read_text(encoding="utf-8")
    referenced = set(re.findall(r'src="(assets/[^"]+)"', text))
    assert referenced, "sanity: the README shows images"
    missing = sorted(p for p in referenced if not (ROOT / p).is_file())
    assert not missing, f"README references images that don't exist: {missing}"


def test_every_gallery_sheet_is_shown_in_the_readme():
    """…and the reverse: a sheet nobody links to is a sheet nobody maintains."""
    text = (ROOT / "README.md").read_text(encoding="utf-8")
    sheets = sorted(p.name for p in (ROOT / "assets").glob("gallery-*.png"))
    assert sheets, "sanity: the galleries have been generated"
    unused = [name for name in sheets if name not in text]
